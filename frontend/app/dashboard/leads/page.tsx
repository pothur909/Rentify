...
export default function LeadsPage() {
  ...
  // main status color (based only on lead.status)
  const getStatusColor = (status: string) => {
    switch (status) {
      case "new":
        return "bg-blue-100 text-blue-700";
  ...
  // main UI status, only from lead.status
  const getUiStatus = (lead: any) => {
    if (lead.status === "contacted") return "contacted";
    if (lead.status === "assigned") return "new";
    if (lead.status === "closed") return "closed";
    return "new"; // treat open as new in UI
  };
  ...
  // label only for contact-history status
  const formatContactStatusLabel = (status: string) => {
    switch (status) {
      case "not_reachable":
        return "Not reachable";
      case "not_received_call":
        return "Not received call";
      case "contacted":
        return "Contacted";
      case "pending_contacted_not_interested":
        return "Pending contacted, not interested";
      default:
        return status;
    }
  };
  ...
  // contact status modal open/close/save
  const openContactModal = (lead: any) => {
    setSelectedLeadForContact(lead);
    setContactStatus(lead.latestContactHistory?.status || "");
    setContactNote("");
  ...
  const handleSaveContactStatus = async () => {
    if (!selectedLeadForContact?._id) return;
    ...
    try {
      ...
      const res = await fetch(
        `${baseurl}/api/leads/${selectedLeadForContact._id}/contact-history`,
        {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
      ...
      const latest = data.data?.latestContactHistory;

      setLeads((prev) =>
        prev.map((l) =>
          l._id === selectedLeadForContact._id
            ? { ...l, latestContactHistory: latest }
            : l
        )
      );

      setSelectedLeadForView((prev: any) =>
        prev && prev._id === selectedLeadForContact._id
          ? { ...prev, latestContactHistory: latest }
          : prev
      );
  ...
  const filteredLeads = leads
    ...
    .filter((lead) => {
      ...
      if (filterStatus === "new") {
        return lead.status === "assigned" || lead.status === "open";
      }
  ...
  return (
    <div className="text-gray-800 w-full max-w-full overflow-x-hidden">
        ...
        <div className="overflow-x-auto w-full">
          <table className="w-full text-xs sm:text-sm">
            ...
            <tbody className="divide-y divide-gray-100">
              {filteredLeads.map((lead) => {
                const uiStatus = getUiStatus(lead);
                const latest = lead.latestContactHistory;
                      ...
                      {latest ? (
                        <div className="space-y-1">
                          <p className="text-xs font-semibold text-gray-800">
                            {formatContactStatusLabel(latest.status)}
                          </p>
                          {latest.note && (
                    ...
                    <td className="px-4 lg:px-6 py-4">
                      <div className="flex items-center gap-2">
                        ...
                        <button
                          ...
                          className="px-3 py-1 text-xs font-semibold text-orange-600 bg-orange-50 rounded-lg hover:bg-orange-100 transition-colors"
                        >
                          Contact status
                        </button>
          ...
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-6 relative">
            ...
            <div className="space-y-2 text-sm text-gray-700">
              ...
              <div>
                ...
                {getUiStatus(selectedLeadForView)}
              </div>

              {selectedLeadForView.latestContactHistory && (
                <>
                  <div>
                    <span className="font-semibold">Contact Status: </span>
                    {formatContactStatusLabel(
                      selectedLeadForView.latestContactHistory.status
                    )}
                  </div>
                  <div>
                    <span className="font-semibold">Contact Note: </span>
                    {selectedLeadForView.latestContactHistory.note || "-"}
                  </div>
                  <div>
                    <span className="font-semibold">Last Contact At: </span>
                    {formatDateTime(
                      selectedLeadForView.latestContactHistory.createdAt
                    )}
                  </div>
                </>
              )}
      ...
      {/* Contact Status Modal */}
      {showContactModal && selectedLeadForContact && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-6 relative">
            <button
            ...
            <p className="text-sm text-gray-500 mb-4">
              Lead: {maskName(selectedLeadForContact)} |{" "}
              {selectedLeadForContact.address ||
                selectedLeadForContact.areaKey}
            </p>
            ...
            <div className="space-y-3">
              <div>
                ...
                >
                  ...
                  <option value="not_received_call">Not received call</option>
                  <option value="contacted">Contacted</option>
                  <option value="pending_contacted_not_interested">
                    Pending contacted, not interested
                  </option>
                </select>
              </div>
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="Add a short note about this attempt..."
                  className="w-full border border-gray-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                />
              </div>
            </div>
            ...
            <div className="mt-4 flex justify-end gap-3">
              ...
              <button
                ...
                disabled={savingContact || !contactStatus}
                className={`px-4 py-2 text-sm rounded-xl text-white ${
                  savingContact || !contactStatus
                    ? "bg-blue-300 cursor-not-allowed"
                    : "bg-blue-600 hover:bg-blue-700"
                }`}
              >
                {savingContact ? "Saving..." : "Save status"}
              </button>
            </div>
          </div>
        </div>
